...
<body> 
    ...
    <?php
        $query1 = $con->query("
        SELECT 
            MONTH(Order_DeliveryTime) as months,
            SUM(Order_ID) as totalOrder
        FROM orderrecord
        GROUP BY months
        ");
    ...
    // === include 'setup' then 'config' above ===
    const label = <?php echo json_encode($months) ?>;
    const data = {
        labels: label,
        datasets: [{
        label: 'Monthly Total Order',
